...
function Dispatcher(task_monitor_port, launcher_url_home_path, db_conn, nodes) {
	...
	function notifyNewJobSubmitted(newJob) {if (typeof me.onNewJobSubmitted === 'function') me.onNewJobSubmitted(newJob);}
	...
	function TrackedJobs(max_done_jobs_tracking) {
		...
		var __jobs = {};
		var __doneJobs = [];
		function notifyNewJobSubmitted(newJob) {if (typeof self.onNewJobSubmitted == 'function') self.onNewJobSubmitted(newJob);}
		...
		this.trackNewJob = function(job_id, num_tasks) {
			__jobs[job_id] = {job_id: job_id, status:'SUBMITTED', num_tasks: num_tasks, num_tasks_finished: 0, complete_pct: 0.0};
			notifyNewJobSubmitted(__jobs[job_id]);
			notifyJobStatusChanged(__jobs[job_id]);
		};
	...
	var trackedJobs = new TrackedJobs(10);	// tracked jobs
	...
	trackedJobs.onNewJobSubmitted = function (newJob) {
		notifyNewJobSubmitted(newJob);
	};
	...
	// submit a job
	this.submitJob = function(job, job_xml, onJobCreated, onError) {
		if (shutdown) {
		...
		var stmt = sqlserver.query(db_conn.conn_str, db_conn.sqls['SubmitJob'], [job_xml]);
		var job_id = null;	
		var ci = null;
		stmt.on('meta', function (meta) {ci = meta;});
		stmt.on('column', function (idx, data, more) {
			var fld = ci[idx].name;
			if (fld == 'job_id') job_id=parseInt(data.toString());
		});
		stmt.on('done', function ()	{
			if (!job_id || isNaN(job_id)) {
				if (typeof onError == 'function') onError('unable to submit job');
			}
			else {
				console.log('job ' + job_id + " created");
				trackedJobs.trackNewJob(job_id, job.tasks.length);
	...
	function getJobStatusDB(job_id, onDone, onError) {
		var stmt = sqlserver.query(db_conn.conn_str, db_conn.sqls['GetJobStatus'], [job_id]);
		var ci = null;
		var job_progress = {job_id: job_id, status: '', num_tasks: 0, num_tasks_finished: 0, complete_pct: 0.0};
		stmt.on('meta', function (meta) {ci = meta;});
		stmt.on('column'
		,function (idx, data, more) {
			var fld = ci[idx].name;
			switch(fld)
			{
				case 'status':
					job_progress.status = data;
					break;
				case 'num_tasks':
					job_progress.num_tasks = data;
					break;
				case 'num_tasks_finished':
					job_progress.num_tasks_finished = data;
					break;
				case 'complete_pct':
					job_progress.complete_pct = data;
					break;
			}
		});
		stmt.on('done', function ()	{
			if (typeof onDone == 'function') onDone(job_progress);
		});
		stmt.on('error', function (err) {
			if (typeof onError == 'function') onError(err.toString());
		});
	...
	this.getJobResult = function(job_id, onDone) {
		...
		var stmt = sqlserver.query(db_conn.conn_str, sql, [job_id], onDone);
	};
}

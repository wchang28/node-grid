...
function Dispatcher(msgBroker, dispatcherToTaskLauncherTopic, taskDispatchQueue, db_conn) {
	var me = this;
	...
	function notifyNodeDisabled(node) {if (typeof me.onNodeDisabled === 'function') me.onNodeDisabled(node);}
	...
	function NodesStatus() {
		...
		function notifyNodeDisabled(node) {if (typeof self.onNodeDisabled == 'function') self.onNodeDisabled(node);}
		...
		function getNodeStatus(nodeItem) {
			...
			var ns = {name: node.name, num_cpus: node.num_cpus, ip: node.ip, available_cpus: nodeItem.available_cpus, active: nodeItem.active, leavePending: nodeItem.leavePending};
			return ns;			
		}
		function nodeNotRunningAnyTask(nodeItem) {
		...
		this.incrementAvailableCPUCount = function(host) {
			...
			if (nodeItem) {
				...
				if (!nodeItem.active && nodeItem.leavePending && nodeNotRunningAnyTask(nodeItem)) {
					delete  __nodes[host];
		...
		this.disableNode = function(host, leaveGrid) {
			if (typeof leaveGrid !== 'boolean') leaveGrid = true;
			var nodeItem = __nodes[host];
			if (nodeItem && nodeItem.active) {
				nodeItem.active = false;
				notifyNodeDisabled(nodeItem.node);
				nodeItem.leavePending = leaveGrid;
				if (leaveGrid && nodeNotRunningAnyTask(nodeItem)) {
					delete  __nodes[host];
					notifyNodeRemoved(nodeItem.node);
				}
				notifyStatusChanged();
		...
		// return 0 if no cpu available
		this.getNumAvailableCPUs = function() {
			var ret = 0;
			for (var host in __nodes) {	// for each node
				var nodeItem = __nodes[host];
				if (nodeItem.active) {
					ret += nodeItem.available_cpus;
				}
			}
			return ret;
		};
	...
	function notifyNodeAcceptTasks(node, accept) {
		var retObj = {method: "nodeAcceptTasks", host: node.name, content:{accept: accept}};
	...
	function dispatchTasksIfNecessary() {
		var availableCPUs = 0;
		var tasks = null;
		if (!me.isDispatchingTasks() && (availableCPUs=ns.getNumAvailableCPUs()) > 0 && (tasks=queue.dequeueTasks(availableCPUs))) {
			//assert(availableCPUs > 0 && tasks.length > 0 && availableCPUs >= tasks.length);
			numResponsesOutstanding = tasks.length;	// numResponsesOutstanding > 0 ==> dispatching
			var numTasksToDispatch = tasks.length;
			function getSendConfirmHandler(i) {
				return (function(receipt_id) {
					if (i+1 < numTasksToDispatch) msgBroker.send(taskDispatchQueue, {persistent: true}, JSON.stringify(tasks[i+1]), getSendConfirmHandler(i+1));
				});
			}
			msgBroker.send(taskDispatchQueue, {persistent: true}, JSON.stringify(tasks[0]), getSendConfirmHandler(0));
		}
	}
	...
	ns.onNodeEnabled = function(node) {
		notifyNodeAcceptTasks(node, true); // notify node to start accepting tasks
		dispatchTasksIfNecessary();	// more cpus become available
		notifyNodeEnabled(node);
	}
	ns.onNodeDisabled = function(node) {
		notifyNodeAcceptTasks(node, false); // notify node to stop accepting tasks
		notifyNodeDisabled(node);
	};
	...
	this.handleTaskLauncherMsg = function(msgObject) {
		...
		switch (msgObject.method) {
			...
			case "nodeAckDispatchedTaskError": {
				...
				onDispatchAck(host, null, exception);
				break;
			}
	...
	// request a node to leave the grid
	this.requestNodeToLeaveGrid = function (host) {ns.disableNode(host, true);};
	// disable a node
	this.disableNode = function (host) {ns.disableNode(host, false);};

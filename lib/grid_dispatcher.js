...
function Dispatcher(task_monitor_port, launcher_url_home_path, db_conn, nodes) {
	...
	function getJobStatusDB(job_id, onDone, onError) {
		...
		,function (idx, data, more) {
			...
			{
				case 'description':
					if (data) job_progress.description = data;
					break;
				case 'cookie':
					if (data) job_progress.cookie = data;
					break;
				case 'status':
	...
	this.getGridState = function() {return {"nodesStatus": ns, "queue": queue, "trackedJobs": trackedJobs};};
}

...
function Dispatcher(msgBroker, dispatcherToTaskLauncherTopic, db_conn) {
	...
	function notifyOnError(err) {if (typeof me.onError === 'function') me.onError(err);}
	...
	function NodesStatus() {
		...
		this.incrementAvailableCPUCount = function(host) {
			...
			if (nodeItem) {
				...
				if (!nodeItem.active && nodeItem.leavePending && nodeIdle(nodeItem)) {
					delete __nodes[host];
		...
		// disable a node
		this.disableNode = function(host) {
			var nodeItem = __nodes[host];
			if (nodeItem && nodeItem.active) {
				nodeItem.active = false;
				nodeItem.leavePending = false;
				notifyNodeDisabled(nodeItem.node);
				notifyStatusChanged();
			}
		};
		// mark node leave pending
		// only call this if dispatcher is not dispatching
		this.markNodeLeavePending = function(host) {
			var nodeItem = __nodes[host];
			if (nodeItem && (nodeItem.active || (!nodeItem.active && !nodeItem.leavePending))) {
				nodeItem.active = false;
				nodeItem.leavePending = true;
				notifyNodeDisabled(nodeItem.node);
				if (nodeIdle(nodeItem)) {
					delete __nodes[host];
					notifyNodeRemoved(nodeItem.node);
				}
				notifyStatusChanged();
	...
	function Queue() {
		...
		this.enqueueTasks = function(tasks) {
			for (var i in tasks) {	// for each task
				var task = tasks[i];
				var job_id = task.job_id;
				if (!__queues[job_id]) __queues[job_id] = [];
				__queues[job_id].push(task);
			}
			notifyTasksEnqueued(tasks);
			notifyChanged();
		};
	...
	function notifyNodeHostGridLeave(node) {
		var o = {method: "nodeGridLeave", host: node.name};
		msgBroker.send(dispatcherToTaskLauncherTopic, {persistent: true}, JSON.stringify(o), function(receipt_id) {});
	}
	...
	function onDispatchTasksCompleted() {
		// assert(numResponsesOutstanding == 0)
		// or assert(!me.isDispatchingTasks())
		console.log('DISPATCH: no more outstanding request :-)'); // ==> not dispatching
		for (var host in leavePendingQueue)
			ns.markNodeLeavePending(host);
		leavePendingQueue = {};
		dispatchTasksIfNecessary();		
	}
	function onNumResponsesOutstandingChanged() {
		console.log('DISPATCH: ' + numResponsesOutstanding + ' request(s) still outstanding');
		if (!numResponsesOutstanding) onDispatchTasksCompleted();		
	}
	...
	function dispatchTasksIfNecessary() {
		...
		if (!me.isDispatchingTasks() && (availableCPUs=ns.getAvailableCPUs()) && (tasks=queue.dequeueTasks(availableCPUs.length))) {
			...
			onNumResponsesOutstandingChanged();
			...
			function getHTTPRspHandler(host, numTasksDispathced) {
				return (function(res) {
					...
					res.on('end', function() {
						if (res.statusCode != 200) {
							notifyOnError(host + ': http returns status code of ' + res.statusCode);
							numResponsesOutstanding -= numTasksDispathced;
							onNumResponsesOutstandingChanged();
						}
						else
							console.log(host + ' received the tasks dispatch: ' + s);
					})
				});				
			}
			function getHTTPErrorHandler(host, numTasksDispathced) {
				return (function(err) {
					notifyOnError(host + ' has problem with the HTTP request: ' + err.toString());
					numResponsesOutstanding -= numTasksDispathced;
					onNumResponsesOutstandingChanged();
				});				
			}
			for (var host in tasksByHost) {	// for each node
				...
				var dispatchedTasks = tasksByHost[host].tasks;
				options.hostname = (node.use_ip ? node.ip : node.name);
				options.port = node.port;
				var s = JSON.stringify(dispatchedTasks);
				options.headers['Content-Length'] = s.length;
				var req = http.request(options, getHTTPRspHandler(host, dispatchedTasks.length));
				req.on('error', getHTTPErrorHandler(host, dispatchedTasks.length));
				req.end(s);
			}
		}
	}
	function onDispatchTaskAck(host, task, error) {
		if (!error)	ns.decrementAvailableCPUCount(host);
	...
	ns.onNodeRemoved = function(nodeRemoved) {
		notifyNodeHostGridLeave(nodeRemoved);
	...
	trackedJobs.onError = notifyOnError;
	...
	this.handleTaskLauncherMsg = function(msgObject) {
		...
		switch (msgObject.method) {
			case "nodeRequestToJoinGrid": {
				...
				var success = ret[0];
				if (!success) retObj.content.exception = ret[1].toString();
			...
			case "nodeAckDispatchedTask": {
				...
				onDispatchTaskAck(task.node, task, null);
				break;
			}
			case "nodeAckDispatchedTaskError": {
				var task = msgContent.task;
				var exception = msgContent.exception;
				notifyOnError('ACK ERROR: ' + task_toString(task) + ' is acked by ' + task.node + ' with error: ' + exception);
				onDispatchTaskAck(task.node, task, exception);
				break;
			}
	...
	this.enableNode = function (host) {
		ns.enableNode(host);
		if (me.isDispatchingTasks() && leavePendingQueue[host])
			delete leavePendingQueue[host];
	};
	// disable a node
	this.disableNode = function (host, leaveGrid) {
		if (!leaveGrid) {	// just disable the node
			ns.disableNode(host);
			if (me.isDispatchingTasks() && leavePendingQueue[host])
				delete leavePendingQueue[host];
		}
		else {	// leave the grid entirely
			if (me.isDispatchingTasks())
				leavePendingQueue[host] = true;
			else
				ns.markNodeLeavePending(host);
		}
	};
	...
	// enqueue tasks
	this.enqueueTasks = function(tasks) {queue.enqueueTasks(tasks);};

...
function Dispatcher(task_monitor_port, launcher_url_home_path, db_conn, nodes) {
	...
	function dataRowToJobProgress(row) {
		...
		job_progress.submit_time = row['submit_time'].toFormat('MMDDYYYYHH24MISS');
		return job_progress;
	}
	// jobs tracking class
	// events supported:
	...
	function TrackedJobs(max_done_jobs_tracking) {
		...
		this.trackNewJob = function(job_id, description, cookie, num_tasks, submitToken) {
			var job_progress = {job_id: job_id, status:'SUBMITTED', num_tasks: num_tasks, num_tasks_finished: 0, complete_pct: 0.0, success: false, submit_time: new Date().toFormat('MMDDYYYYHH24MISS')};
